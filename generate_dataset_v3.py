...
cfg = {
       ...
       'newDataDirectory' : '/mnt/store/AR_Dataset/Lat60_Lon60_Nans0/', # String for new directory path with trailing /
       'dataDirectory': '/mnt/store/AR_Dataset/active_regions/', # Path to fits data in NOAA AR number folders with trailing /
       'srsDirectory' : '/mnt/store/AR_Dataset/SRS/', # Path to SRS data with trailing /
       'eventDirectory': '/mnt/store/AR_Dataset/Events/', # Path to Events data with trailing / 
       'labelFilePath': '/mnt/store/AR_Dataset/C1.0_24hr_Labels.txt' # Path to label file
       }
print(cfg)
        ...
        for event_file in event_files:
            with open(event_file,'r') as f:
                for line in f:
                    ...
                    if 'XRA' in line:
                        a = line.split(' ')
                        peakTime = (a[8]+a[9]+a[10]).rstrip()
                        noaaNum = a[-1].rstrip()
                        for item in a:
                            if (('A' in item) or ('B' in item) or\
                                ('C' in item) or ('M' in item) or\
                                ('X' in item)) and (not ('1-8A' in item)) and\
                                (not ('XRA' in item)):
                                flare = item
...
def createLabelCsv( filesMoved, cfg ):
    ...
    if os.path.exists(cfg['classification']['flareSize']+'_'+\
                      ...
                      'hr_flareFiles.txt'):
        print('Using exisiting file list for '+
...
if os.path.exists('nans'+str(cfg['nans'])+'_files.txt'):
    print('Using exisiting file list for NaNs allowed: '+str(cfg['nans']))
...
if os.path.exists('eventList.txt'):
    ...
    print('To generate a new event list, deete existing file eventList.txt')
    print('')
else:
    createEventList(cfg)
if os.path.exists(cfg['labelFilePath']):
    print('Using exisiting file list for creation of flare labels')
    print('To generate a new file list, delete existing file '+\
           cfg['labelFilePath'])
    print('')
else:
